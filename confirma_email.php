...
if (!empty($chave)) {
    ...
    $query = "SELECT id FROM usuarios WHERE chave = ? LIMIT 1";
    ...
    if ($result_usuario->num_rows > 0) {
        $result_usuario->bind_result($ID); // Renomeando para $ID para manter a consistência
        $result_usuario->fetch();

        $result_usuario->close(); // Feche a primeira consulta antes de prosseguir

        $query_up_usuario = "UPDATE usuarios SET sit_usuario_id = 1, chave = NULL WHERE id = ?";
        $up_usuario = $conn->prepare($query_up_usuario);
        $up_usuario->bind_param("i", $ID); // Usando $ID para a atualização
        $up_usuario->execute();
        if($up_usuario->execute()){
            ...
            <div class="success__title">Email confirmed!!</div>
    ...
    <div class="error__title">Error: Invalid Address.</div>
    <div class="error__close"><svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 20 20" height="20"><path fill="#393a37" d="m15.8333 5.34166-1.175-1.175-4.6583 4.65834-4.65833-4.65834-1.175 1.175 4.65833 4.65834-4.65833 4.6583 1.175 1.175 4.65833-4.6583 4.6583 4.6583 1.175-1.175-4.6583-4.6583z"></path></svg></div>
    </div>';
    header("Location: login.php");
}
